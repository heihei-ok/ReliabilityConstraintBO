...
info = data_load(r'./data/data_train.xlsx', baseline)
fea = info.features
fidelity = info.info['fidelity']
if lab_fid:
    ori_lab = info.lab
    # plot_pareto(ori_lab.to_numpy())
    # lab添加置信度
    lab= torch.from_numpy(ori_lab.to_numpy()) * (( (torch.from_numpy(fidelity.to_numpy()).unsqueeze(1)-0.5) / 0.3)*0.1+0.8)
    # plot_two_pareto(ori_lab.to_numpy(),lab.numpy())
